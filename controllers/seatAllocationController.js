...
const directoryPath = path.resolve(__dirname, '../arrangementExcels');
const fileNameRegex = /^S[1-8]_+([0][1-9]|[12][0-9]|3[01])-([0][1-9]|[1][0-2])+-([0-9]{4})+_[AP]M\.xlsx$/;
...
const getExams = async (req, res) => {
    ...
    try {
        ...
        const schedules = await Schedule.find({ date: dateObject, time: time, user: user }).select('sem branch subject subcode').lean();

        const exams = schedules.reduce((acc, { sem, branch, subject }) => {
            const exam = `S${sem}-${branch}-${subject}`;
            acc.push(exam);
            return acc;
        }, []);
        ...
        const details = schedules.map(({ sem, branch, subject, subcode }) => {
            return { sem, branch, subject, subcode };
        });        
...
function extractSemNumber(path) {
    // Regular expression to match 'S' followed by a digit (S2 in this case)
    const regex = /S(\d)/;
    const match = path.match(regex);
    
    if (match) {
        return match[1]; // Return the matched SEM number
    } else {
...
const sendExcels = async (req, res) => {
    ...
    try {
        ...
        for (const item of attachments) {
            ...
            const sem = extractSemNumber(item.path);
            const obj = await Email.findOne({sem}, { emails: 1, _id: 0 });
            ...
            for (const email of emailList) {
                ...
                const mailOptions = {
                    ...
                    attachments: item  // Assuming attachments is an array
                };
